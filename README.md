...
2. 程序启动后会自动扫描。优先选择名称后带 `★` 的设备；星号表示其广播数据包含 `FFF0` 服务。
...
只有设备完成全部握手并进入“已就绪”状态后，程序才会把它记为最后连接设备。配置保存在：

```text
~/Library/Application Support/BLEAssist/settings.json
```

下次启动时，程序会自动扫描，并优先按 macOS CoreBluetooth 设备 UUID 选中该设备；UUID 未找到时再按设备名称匹配。程序不会自动发起连接。要清除记录，退出程序后删除上述文件即可。
